...
%jpl_call_1( array(_T), X, Mspec, Args, Rx) :-
%  %	only one array method is supported (and handled as a special case)
%	(	Mspec == getLength,
%		Args == []
%	->	jGetArrayLength( X, Rx)
%	;	integer( Mspec),
%		Mspec >= 0,
%		Args == []
%	->	fail					% maybe we oughta get the Mspec-th element?
%	).
...
% qp_atom_chars( A, Cs) :-
%	atom_chars/2 has different QP/ISO semantics,
%	so all references to the old version (atom to ASCII codes)
%	are being replaced by calls to this one.
%	NB atom_codes/2 is native in SWI-Prolog 3.3.0 onwards (and in ISO)
%	and is emulated in my SWI-Prolog 3.2.8 plwin.rc

qp_atom_chars( A, Cs) :-
  %	(	old_swi
  %	  %	->	Cs \== [],
  %	 	atom_codes( A, Cs)
  %	;	catch(	atom_codes( A, Cs),
  %			_,
  %			fail
  %		)
  %	).
	catch(	atom_codes( A, Cs),			% scrapped backwards compatibility 5/4/2000
		_,
		fail
	).
